...
if [[ -z "${_DOWN}" ]]; then 
    ...
    devcontainer up --id-label containername=${_CONTAINERNAME} --remove-existing-container ${_NOCACHE} --config "$(pwd)/.devcontainer/${_CONTAINERNAME}/devcontainer.json" --workspace-folder "$(pwd)/.devcontainer/${_CONTAINERNAME}"
else
